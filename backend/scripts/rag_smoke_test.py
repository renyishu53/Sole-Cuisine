...
不依赖 pytest 的 stub/mock，直接连 docker-compose 起的 Chroma + Neo4j 底座，
调用 KnowledgeService 播种知识库并调用 knowledge.retrieve() 验证：
  - chroma_status == "connected" 且 vector_hits 非空（真实向量召回）
  - neo4j_status == "connected" 且图谱实体已同步（真实图检索底座）

运行（需先 docker compose up -d mysql redis neo4j chroma）：
    cd backend
...
async def main() -> int:
    # .env 已指向 localhost:8001(chroma)/7687(neo4j)/6379(redis)。
    # 强制 LLM=demo，使 query 改写 / 实体抽取走确定性规则回退，
    # 隔离检索基础设施测试、避免依赖外部 LLM 网络。
    settings = get_settings()
    settings.llm_provider = "demo"
    print(
        f"[config] chroma={settings.chroma_host}:{settings.chroma_port} "
    ...
    print(
        f"[status] chroma={status.chroma} neo4j={status.neo4j} "
        f"embedding={status.embedding} collection={status.collection}"
    )

    # 2) 播种知识库：ingest_text 同时写 Chroma(向量) + Neo4j(图谱实体)
    print(f"[seed] 播种 {len(BOOTSTRAP_DOCUMENTS)} 篇文档到 Chroma + Neo4j ...")
    ...
    print(
        f"[consistency] chroma_docs={cons.chroma_documents} "
    ...
    for q in queries:
        ...
        v_ok = bool(bundle.vector_hits) and bundle.chroma_status == "connected"
        ...
        print(f"\n[retrieve] {q!r}")
        print(
            f"  chroma_status={bundle.chroma_status} "
    ...
    print("\n=== RESULT ===")
    all_ok = all(ok for _, ok, _ in checks) and status.chroma == "connected"
    for q, ok, extra in checks:
        print(f"  [{'PASS' if ok else 'FAIL'}] {q} {'' if ok else '(' + extra + ')'}")
    if all_ok:
        print("PASS: 真实端到端检索成功（Chroma 已连接且有命中，Neo4j 已连接）")
        return 0
    print("FAIL: 真实检索未达预期")
    return 1
